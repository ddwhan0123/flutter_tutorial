            ...
            return FadeTransition(
              ...
              child: CupertinoPopover(
                attachRect: Rect.fromLTWH(
                    offset.dx, 8, bounds.width, bounds.height),
                child: body,
...
class CupertinoPopover extends StatefulWidget {
  ...
  CupertinoPopover(
    ...
    BoxConstraints temp;
    if (constraints != null) {
      temp = BoxConstraints(maxHeight: 160.0, maxWidth: 130.0).copyWith(
      ...
      temp = BoxConstraints(maxHeight: 160.0, maxWidth: 130.0);
    }
...
class _CupertionPopoverPosition extends SingleChildRenderObjectWidget {
  ...
  _CupertionPopoverPosition(
      {Widget child, this.attachRect, this.constraints, this.scale})
      : super(child: child);

  @override
  RenderObject createRenderObject(BuildContext context) =>
      _CupertionPopoverPositionRenderObject(
          attachRect: attachRect, constraints: constraints);
...
class _CupertionPopoverPositionRenderObject extends RenderShiftedBox {
  ...
  Offset calcOffset(Size size) {
    ...
    var isArrowUp = ScreenUtil.getInstance().screenHeight >
        attachRect.bottom + size.height + CupertinoPopoverState._arrowHeight;

    if (attachRect.left > size.width / 2 &&
        ScreenUtil.getInstance().screenWidth - attachRect.right >
            size.width / 2) {
      //判断是否可以在中间
    ...
    if (isArrowUp) {
      ...
      return Offset(bodyLeft,
          attachRect.top - size.height - CupertinoPopoverState._arrowHeight);
    }
  }
...
class _CupertionPopoverContext extends SingleChildRenderObjectWidget {
  ...
  _CupertionPopoverContext(
      {Widget child, this.attachRect, this.color, this.scale, this.radius})
      : super(child: child);

  @override
  RenderObject createRenderObject(BuildContext context) =>
      _CupertionPopoverContextRenderObject(
          attachRect: attachRect, color: color, scale: scale, radius: radius);
...
//
    ...
    var arrowLeft = attachRect.left +
        attachRect.width / 2 -
        CupertinoPopoverState._arrowWidth / 2 -
        offset.dx;
    var translation = Offset(arrowLeft + CupertinoPopoverState._arrowWidth / 2,
        isArrowUp ? 0.0 : size.height);
    ...
    Rect bodyRect =
        Offset(0.0, isArrowUp ? CupertinoPopoverState._arrowHeight : 0.0) &
            child.size;

    context.pushClipPath(needsCompositing, offset, offset & size,
